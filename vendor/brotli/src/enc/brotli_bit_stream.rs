...
fn BrotliWriteBits(n_bits: u8, bits: u64, pos: &mut usize, array: &mut [u8]) {
    debug_assert_eq!(bits >> n_bits, 0);
    debug_assert!(n_bits <= 56);
    let ptr_offset: usize = ((*pos >> 3) as u32) as usize;
    let mut v = array[ptr_offset] as u64;
    v |= bits << ((*pos) as u64 & 7);
    // mvq-форк: один u64 LE-store вместо 8 bounds-checked byte-store; bit-exact на LE.
    // asserts → debug_assert (в release не стрипались, а это per-symbol hot).
    array[ptr_offset..ptr_offset + 8].copy_from_slice(&v.to_le_bytes());
    *pos += n_bits as usize
}
