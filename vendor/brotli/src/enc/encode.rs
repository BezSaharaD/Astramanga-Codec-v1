...
pub static BR_PROF: [core::sync::atomic::AtomicU64; 3] = [
    ...
    ),
{
    ...
    if params.quality <= 2 {
        ...
        if params.quality < 10i32 {
            ...
            #[cfg(feature = "std")] let __brt1 = std::time::Instant::now();
            ...
            #[cfg(feature = "std")] crate::enc::encode::BR_PROF[1].fetch_add(__brt1.elapsed().as_micros() as u64, core::sync::atomic::Ordering::Relaxed);
        }
        ...
        #[cfg(feature = "std")] let __brt2 = std::time::Instant::now();
        store_meta_block(
            alloc,
            data,
            wrapped_last_flush_pos as usize,
        ...
        #[cfg(feature = "std")] crate::enc::encode::BR_PROF[2].fetch_add(__brt2.elapsed().as_micros() as u64, core::sync::atomic::Ordering::Relaxed);
        mb.destroy(alloc);
    }
...
impl<Alloc: BrotliAlloc> BrotliEncoderStateStruct<Alloc> {
        ...
        ),
    {
        ...
        #[cfg(feature = "std")] let __brt0 = std::time::Instant::now();
        ...
        #[cfg(feature = "std")] BR_PROF[0].fetch_add(__brt0.elapsed().as_micros() as u64, core::sync::atomic::Ordering::Relaxed);
        {
