...
>(
    ...
    // mvq-форк: zip вместо индексов h0[i]/h1[i] — снимает per-элемент bounds-check, автовек. bit-exact.
    for (d, s) in h0[..n].iter_mut().zip(h1[..n].iter()) {
        *d = d.wrapping_add(*s);
    }
}
...
>(
    ...
    // mvq-форк: hoist через split_at_mut (было 3 trait-dispatch + bounds/итерацию + дубль h0/h0a),
    // zip → автовек. bit-exact (та же целочисл. сумма; вызывающий cluster.rs:170 всегда i0≠i1).
    let (dst, src) = if i0 < i1 {
        let (l, r) = xself.split_at_mut(i1);
        (&mut l[i0], &r[0])
    } else {
        let (l, r) = xself.split_at_mut(i0);
        (&mut r[0], &l[i1])
    };
    let h0 = dst.slice_mut();
    let h1 = src.slice();
    let n = min(h0.len(), h1.len());
    for (d, s) in h0[..n].iter_mut().zip(h1[..n].iter()) {
        *d = d.wrapping_add(*s);
