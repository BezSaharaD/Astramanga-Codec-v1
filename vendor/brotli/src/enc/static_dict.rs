...
// mvq-форк: делегируем на word-based Complex (8-байт блоки) вместо побайтового цикла,
// который автор пометил "factor of 5 slower". bit-exact — та же длина совпадения
// (контракт вызывающего s1.len()>=limit сохраняется: Complex читает 8 байт лишь при limit>=8).
#[allow(unused)]
pub fn FindMatchLengthWithLimit(s1: &[u8], s2: &[u8], limit: usize) -> usize {
    ComplexFindMatchLengthWithLimit(s1, s2, limit)
}
#[allow(unused)]
