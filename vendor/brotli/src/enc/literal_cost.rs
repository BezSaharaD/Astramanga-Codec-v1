...
pub fn BrotliEstimateBitCostsForLiterals(
    ...
    if is_mostly_utf8(data, pos, mask, len, kMinUTF8Ratio) {
        ...
        i = 0usize;
        // mvq [A8]: FastLog2f64(in_window) — медленный log2 (in_window обычно >256) вызывался per-byte,
        // хотя in_window стабилен в середине (окно полное 2000, оба края движутся синхронно). Кэшируем,
        // пересчёт только при РЕАЛЬНОМ изменении in_window (края потока). bit-exact (то же значение).
        let mut log_in = FastLog2f64(in_window as u64) as f64;
        while i < len {
            {
                let mut histo: usize;
                let iw_before = in_window;
                if i >= window_half {
                    {
                        let _rhs = 1;
                ...
                if in_window != iw_before {
                    log_in = FastLog2f64(in_window as u64) as f64;
                }
                ...
                }
                {
                    //precision is vital here: lets keep double precision
                    let mut lit_cost: f64 =
                        log_in - FastLog2f64(histo as u64) as f64;
                    lit_cost += 0.029;
                    if lit_cost < 1.0 {
                        lit_cost *= 0.5;
                        lit_cost += 0.5;
