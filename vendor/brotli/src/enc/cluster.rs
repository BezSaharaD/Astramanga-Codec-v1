...
>(
    ...
    if idx1 == idx2 {
        ...
        if (out[idx1 as usize]).total_count() == 0usize {
            ...
            // mvq-форк: BrotliPopulationCost >= 0, поэтому cost_combo < (threshold - cost_diff)
            // невозможно при (threshold - cost_diff) <= 0 → пропускаем дорогой clone+add+cost.
            // bit-exact: пропускаются только заведомо не-good пары (то же множество принятых).
            if threshold - p.cost_diff > 0.0 as super::util::floatX {
...
>(
    ...
    let max_input_histograms: usize = 16usize; // mvq: 64→16 (свип 2026-07-08: build_metablock pick −26%/blade −22% за вес +<0.02%, выгода ~50× по курсу; колено ~16, ниже плато). Откат=64.
