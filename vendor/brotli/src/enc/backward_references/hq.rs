...
fn MaxZopfliLen(params: &BrotliEncoderParams) -> usize {
    // mvq: q10 порог 150→40 (свип 2026-07-08: матчи >40 редки в rz/coeff-потоках → длинные форсятся skip'ом
    // раньше, минуя cost-DP; backward_refs pick −6%/blade −9% за вес +0.09%/+0.06%, выгода 7-12× по курсу 1%≈10мс). Откат = 150.
...
impl<AllocF: Allocator<floatX>> ZopfliCostModel<AllocF> {
    ...
    fn set_from_literal_costs(
        ...
        // mvq [A5]: cost_cmd/cost_dist — КОНСТАНТЫ (FastLog2(11+i)/(20+i)), не зависят от данных.
        // Предвычисляем раз в static LUT (было 704+dist FastLog2 на КАЖДЫЙ set_from_literal, per-метаблок). bit-exact.
        {
            use std::sync::OnceLock;
            static CMD_LUT: OnceLock<[floatX; BROTLI_NUM_COMMAND_SYMBOLS]> = OnceLock::new();
            let lut = CMD_LUT.get_or_init(|| {
                let mut a = [0.0 as floatX; BROTLI_NUM_COMMAND_SYMBOLS];
                for i in 0..BROTLI_NUM_COMMAND_SYMBOLS {
                    a[i] = FastLog2(11 + i as u64);
                }
                a
            });
            cost_cmd.copy_from_slice(&lut[..]);
        }
        {
            use std::sync::OnceLock;
            const MAXD: usize = 544;
            static DIST_LUT: OnceLock<[floatX; MAXD]> = OnceLock::new();
            let lut = DIST_LUT.get_or_init(|| {
                let mut a = [0.0 as floatX; MAXD];
                for i in 0..MAXD {
                    a[i] = FastLog2((20u64).wrapping_add(i as u64)) as floatX;
                }
                a
            });
            let ds = self.distance_histogram_size as usize;
            cost_dist[..ds].copy_from_slice(&lut[..ds]);
        }
    ...
    Buckets: PartialEq<Buckets>,
{
    ...
    }
    {
        ...
        if false // mvq-форк: static dict бесполезен бинарным потокам, отключён (−4% скорости, вес идентичен)
            && dictionary.is_some()
...
fn EvaluateNode<AllocF: Allocator<floatX>>(
    ...
    let lit0 = model.get_literal_costs(0, pos); // mvq: было 2 вызова (условие + costdiff) — кэш, bit-exact
    if node_cost <= lit0 {
        let mut posdata = PosData {
            pos,
            cost: node_cost,
            costdiff: node_cost - lit0,
            distance_cache: [0; 4],
        };
...
fn UpdateNodes<AllocF: Allocator<floatX>>(
    ...
    for k in 0..min(MaxZopfliCandidates(params), queue.size()) {
        ...
        for j in 0..16 {
            ...
            // mvq: copycode/cmdcode/cost меняются лишь на границах copy-length-бакетов (GetCopyLengthCode),
            // не на каждый l → кэшируем cost пока copycode тот же. bit-exact (значение cost идентично).
            ...
            for l in best_len.wrapping_add(1)..=len {
                ...
                if copycode != last_cc {
                    ...
                    cost = (if cmdcode < 128 { base_cost } else { dist_cost })
